...
window.addEventListener('DOMContentLoaded', function() {
   ...
   const milestones = [
      {pct:6, text: 'Loaded fonts'},
      {pct:18, text: 'Initialized projection (d3)'} ,
      {pct:30, text: 'Loaded country list (restcountries)'},
      {pct:44, text: 'Loaded topojson/world-atlas'},
      {pct:56, text: 'Prepared flags dataset'},
      {pct:68, text: 'Loaded assets (icons & images)'},
      {pct:78, text: 'Wired handlers (Countries Mode)'} ,
      {pct:82, text: 'Wired handlers (Cities Mode)'},
      {pct:84, text: 'Wired handlers (Flags mode)'},
      {pct:90, text: 'Built Quizes'},
      {pct:94, text: 'Finalising.'},
      {pct:95, text: 'Finalising..'},
      {pct:96, text: 'Finalising...'},
      {pct:97, text: 'Finalising.'},
      {pct:98, text: 'Finalising..'},
      {pct:99, text: 'Finalising...'},
      {pct:100, text: 'Ready'}
   ];

   let pct = 0; let milestoneIndex = 0;
   const totalDuration = 3200; // ms - visual length of the fake load
   const start = Date.now();

   // Use an interval to simulate progress with slight random stalls for realism
   const tickInterval = 70; // ms
   const interval = setInterval(() => {
      ...
      if (target > 40 && target < 50) target = Math.min(48, target);
      ...
      if (pct >= 100 || elapsed > totalDuration + 800) {
         ...
         setTimeout(() => { logo.style.opacity = '0'; splash.style.opacity = '0'; }, 200);
         setTimeout(() => {
...
function startGame(mode){
   gameMode=mode;
   document.getElementById("start-screen").style.display="none";
   round=1; noregdip=0; wrongGuesses=0; revealedCountries.clear(); revealedCapitals.clear();
...
function revealCountry(rec) {
    ...
    // Countries that should use stretched placement
    const forceStretch = new Set(["RUS"]);

    let feature = getFeature(rec);

    // PATCH: handle missing/odd territories like British Somaliland and Northern Cyprus
    if (!feature) {
        if (rec.cca3 === "BSC") { // British Somaliland
            const topoFeature = worldData.objects.countries.geometries.find(d => d.id === 686);
            if (topoFeature) {
                feature = {
                    type: "Feature",
                    geometry: topoFeature.geometry,
                    properties: { name: rec.name, iso_a3: rec.cca3 }
                };
            }
        } else if (rec.cca3 === "NCY") { // Northern Cyprus
            const topoFeature = worldData.objects.countries.geometries.find(d => d.id === 0); // fixed nested .geometries
            if (topoFeature) {
                feature = {
                    type: "Feature",
                    geometry: topoFeature.geometry,
    ...
    function placeImageCover(imgSel, naturalImg, bounds) {
        ...
        imgSel
            .attr("width", width)
            .attr("height", height)
            .attr("x", bounds[0][0] - (width - wBox) / 2)
            .attr("y", bounds[0][1] - (height - hBox) / 2)
    ...
    function renderSingleFeature(feature, rec, clipId, stretchOverride = false) {
        ...
        function applyStretch(imgSel, b) {
            imgSel
                ...
                .attr("preserveAspectRatio", "none"); // ensures actual stretch
        }
        ...
        if (flagImg && flagImg.complete && flagImg.naturalWidth) {
            ...
            tmp.onerror = function() { d3.select(img.node()).remove(); };
            tmp.src = flagSrc;
        }
    }
    ...
    function processMultiPolygon(feature, rec) {
        ...
        if (rec.cca3 === "FRA") {
            // Detect Corsica polygon by approximate centroid (lon/lat bounds)
    ...
    } else if (["USA","FRA","NLD","PRT","ESP","TWN","MLT","AUS"].includes(rec.cca3) && feature.geometry.type === "MultiPolygon") {
        processMultiPolygon(feature, rec);
    } else {
...
function nextRound(){
   ...
   if(gameMode==="Normal"||gameMode==="Hard"){
      ...
      if(noregdip>=continentOrder.length){ endGame(); return; }
   }else{
